...
import classifier.sample.{LabeledSample, ObjectClass, Property}
...
private[naiveBayes] case class Fit(sample: LabeledSample = new LabeledSample) {
  ...
  def classSpecificWordCount(w: Property, cl: ObjectClass): Int = sample.classWordList(cl).count(_.equals(w))
